...
Secondly, in their paper, Klemperer and Meyer show that in the limit of an diverging upper bound for their shocks, their continuum of solutions converges towards a unique solution. Our unique solution in the limit of small dynamic costs is the same as that ok KM in the limit of infinite support of demand shocks.\\
...
This result sheds some light on one of the questions that the literature on electricity market focuses on. \\

Accounting for dynamic costs induces a collapse of the equilibria set from a continuum to a unique element. \\
...
%
%In addition to this consideration, one must note that our unique solution rotates towards a steeper and steeper supply schedule as the dynamic costs increase. However this rotation does not occur around the mean demand shock, but around a lower shock value. This means that, in expected terms, our solutions are more costly than the one selected by KMs infinite support argument. As long as the dynamic costs are not too high, our solution lies in the region of the price quantity plane delimited by the continuum of solutions of KM. Therefore, for empirical work trying to estimate
...
We think that this strand of argument trying to explain bids' dynamics in the light of the supply function equilibria framework is unsatisfying and we argue that forecasting demand becomes important for firms when one considers dynamic effects, that is effects that are history dependent, of which dynamic costs which we model in this paper are an instance (one can think of start-up and shut-down costs as another instance of such dynamic effects). \\
...
In the case of an envelope evolving with time, that is shocks belong to the bounded support $[\hat{\theta}(t)-\omega(t),\hat{\theta}(t)+\omega(t)]$, there exists a unique optimal solution to the monopoly problem. It can be expressed as as surface in the price-quantity-time space:
\begin{flalign}
&p^*(\theta(t),t)=\frac{4\gamma\left(1+\frac{\tau}{\omega}\frac{d\omega}{dt}(t)\right)+1+\lambda}{4\gamma\left(1+\frac{\tau}{\omega}\frac{d\omega}{dt}(t)\right)+2+\lambda}\cdot\theta(t)-\frac{4\gamma\left(1+\frac{\tau}{\omega}\frac{d\omega}{dt}(t)\right)}{(2+\lambda)\left(4\gamma\left(1+\frac{\tau}{\omega}\frac{d\omega}{dt}(t)\right)+1+\lambda\right)}\cdot\hat{\theta}(t)&\label{monopdynp}
\end{flalign}
...
Note that if $\frac{d\omega}{dt}=0$ equations \ref{monopdynp} and \ref{monopdynS} are equal to equations \ref{monopsol} and \ref{monopS} respectively as expected. Note also that the solution is exactly the same as in the static monopoly case in which one replaces the dynamic cost parameter $\gamma$ by $\gamma\left(1+\frac{\tau}{\omega}\frac{d\omega}{dt}(t)\right)$. The optimal supply schedule is shifted uniformly following the expected shock $\hat{\theta}(t)$, and its slope behaves as if the dynamic cost parameter was fluctuating with the width of the bounds of the shocks. The transition between a low uncertainty region to a higher uncertainty one behaves as if during the transient regime the dynamic cost parameter had a higher value, implying a higher slope. \\
